...
func NewServercfg(addr string, handler http.Handler, tlsconfig *tls.Config) (srv *Server) {
	regLock.Lock()
...
func ListenAndServeTLSCFG(addr string, handler http.Handler, tlsconfig *tls.Config) error {
	server := NewServercfg(addr, handler, tlsconfig)
